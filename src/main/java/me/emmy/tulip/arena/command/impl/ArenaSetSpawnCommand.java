...
public class ArenaSetSpawnCommand extends BaseCommand {
    ...
    public void onCommand(CommandArgs command) {
        ...
        if (command.length() < 1) {
            player.sendMessage(CC.translate("&6Usage: &e/arena setspawn &b<name>"));
            return;
        }
        ...
        player.sendMessage(CC.translate(Locale.ARENA_SPAWN_SET.getStringPath()).replace("{arena}", arenaName));
    }
}
