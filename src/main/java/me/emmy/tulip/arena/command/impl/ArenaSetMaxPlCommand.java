 ...
 * Created by Emmy
 * Project: Tulip
 * Date: 05/06/2024 - 19:30
 */
public class ArenaSetMaxPlCommand extends BaseCommand {
    @Override
    @Command(name = "arena.setmaxplayers", permission = "Tulip.admin")
    public void onCommand(CommandArgs command) {
        Player player = command.getPlayer();
        String[] args = command.getArgs();

        if (command.length() < 2) {
            player.sendMessage(CC.translate("&cUsage: /arena setmaxplayers <name> <maxPlayers>"));
            return;
        }
        ...
        Tulip.getInstance().getArenaRepository().getArena(arenaName).setMaxPlayers(Integer.parseInt(args[1]));
        Tulip.getInstance().getArenaRepository().saveArena(arena);
        player.sendMessage(CC.translate("&aMax players set to " + args[1] + " for arena " + arenaName + "!"));
